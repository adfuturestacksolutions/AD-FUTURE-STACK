  ...
  return (
    <div className="bg-white rounded-3xl p-8 flex flex-col justify-between border border-slate-200 shadow-md hover:border-violet-500/50 hover:shadow-xl transition-all duration-300 relative overflow-hidden">
      ...
      <div>
        ...
        <span className="text-sm font-bold text-violet-700 block mb-4">{solution.subtitle}</span>
        ...
        <div className="space-y-3 mb-8 pt-4 border-t border-slate-100">
          ...
          {solution.features.map((feat) => (
            <div key={feat} className="flex items-center gap-2.5 text-xs text-slate-700 font-medium">
              <CheckCircle2 className="h-4 w-4 text-violet-700 shrink-0" />
              <span>{feat}</span>
            </div>
          ))}
        </div>
      ...
      <Link
        to="/contact"
        className="w-full inline-flex items-center justify-center gap-2 bg-slate-900 border border-slate-800 hover:bg-violet-700 hover:border-violet-700 text-white font-bold text-xs uppercase tracking-wider py-3.5 px-6 rounded-xl transition-all"
      >
